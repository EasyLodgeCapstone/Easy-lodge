...
class AuthController {
  async registerUser(req, res, next) {
    try {
      ...
      const user = await userServiceActivities.createUser({
        name,
        email,
        password,
      });
  ...
  async loginUser(req, res, next) {
    try {
      ...
      res.status(200).json({
        success: true,
        message:
          "Login successful. Please verify your email if you haven't already.",
        data: result,
      });
    } catch (error) {
      next(error);
  ...
  async refreshToken(req, res, next) {
    try {
      const token = await userServiceActivities.refreshAccessToken(
        req.user.id,
        req.refreshToken,
      );
      res.status(200).json({
